...
| [Quantum Machine Learning for Passive Sonar](papers/qml_passive_sonar/). Source: Bach & Nguyen (2025 preprint) | Reproduction of the SAV spectral feature extractor, DEMON baseline, CNN/HQ-CNN classifier, and a MerLin photonic variant for passive-sonar vessel classification. The scaffold runs end-to-end with synthetic ShipsEar-like data; DeepShip sanity runs auto-download a small public sample if no local WAVs are present, while full-dataset instructions and contacts live under `data/qml_passive_sonar/README.md`. |
