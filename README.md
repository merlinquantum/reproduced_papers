...
| [Quantum Latent Distributions in Deep Generative Models](papers/quantum_latent_distributions/). Source: [bacarreza_2025](https://arxiv.org/abs/2508.19857) | The paper uses a boson sampler as the *latent* distribution of a GAN rather than as a trainable layer, so no gradient crosses the quantum boundary and the quantum cost is a one-off bank of samples. Table I reproduces: on the synthetic discrete dataset the boson sampler beats its own distinguishable-photon control on every seed, by +0.0124 ± 0.0045 at 5k iterations and +0.0059 ± 0.0029 at 20k, converging on the paper's +0.005 at 40k. The gap narrows as the training budget grows, matching the authors' own Appendix C remark that longer training lets the distinguishable latent catch up. On the mixture-of-Gaussians dataset the qualitative claim reproduces cleanly (the Gaussian latent interpolates 3x more between modes than any other, on every seed) but the ranking does not: the boson sampler is beaten there by its distinguishable control and by Bernoulli. QM9 is implemented and not yet run. |
