...
| [Quantum-Enhanced Generative Models for Rare Event Prediction](papers/QEGM_rare_events/). Source: [Haider_2025](https://arxiv.org/abs/2511.02042) | The paper claims a hybrid VAE + variational quantum circuit improves rare-event tail KL by ~50% over GAN/VAE/Diffusion baselines on a synthetic Gaussian-mixture benchmark. Under two controlled ablations our reproduction **refutes** both proposed mechanisms: (1) pinning the VQC output to a constant `r = 0.5` yields tail KL 0.177 ± 0.048, statistically indistinguishable from the trained gate VQC (0.208 ± 0.033) — Eq. 7 is provably inert; (2) removing the hybrid tail-aware loss term *improves* tail KL for every variant (VAE −16%, gate QEGM −41%, MerLin QEGM −6%) — Eq. 5 is mildly counterproductive. The paper additionally has structural red flags (Index Terms copy-pasted from the lead author's blockchain paper, mislabelled architecture figure, internal inconsistency between Eqs. 12 and 13, no released code). Real-world finance/climate/protein experiments blocked by missing dataset URLs and hyperparameters. Verdict: do not pursue.|
