...
| [Fourier Fingerprints of Ansatzes in Quantum Machine Learning](papers/Fourier_Fingerprints/). Source: [Strobl_2025](https://arxiv.org/abs/2508.20868) | The fingerprint construction and the FCC metric carry over to linear optics: every circuit and encoding yields a well-formed fingerprint, and the encoding scale factors set the accessible spectrum (up to frequency 31 for exponential against 15 for linear). The paper's predictive claim does not reproduce on our circuit set, where the highest-FCC circuit attains the lowest error. That comparison is confounded: the four topologies span 8 to 60 parameters, and capacity ranks the error far better (Spearman -0.88) than FCC does (-0.22). A capacity-matched circuit set is needed to test the claim properly. |
