...
| [Fourier Fingerprints of Ansatzes in Quantum Machine Learning](papers/Fourier_Fingerprints/). Source: [Strobl_2025](https://arxiv.org/abs/2508.20868) | The paper's diagnostic for ansatz selection, ported to linear optics. Fingerprints and the FCC metric are computed for photonic circuits by the same definition, and the tool discriminates between topologies. The encoding is shown to set the accessible spectrum: exponential scale factors reach frequency 31 against 15 for a linear ramp, and applying the encoding twice widens it further. Using FCC to select an ansatz is left open, since our four topologies span 8 to 60 parameters and capacity ranks the learning error far better (Spearman -0.88) than FCC does (-0.22); a capacity-matched circuit family is needed for that. |
