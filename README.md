...
| [Continuous-Variable Quantum PINN for PDEs](papers/CV_QPINN_PDE/). Source: [panichi_2025](https://arxiv.org/abs/2503.12244) | The paper introduces a *consistency-loss* CV-QPINN that avoids nested autograd for second-order PDEs and demonstrates it on 1D Poisson (RMSE 1.09e-4) and the 1D heat equation (RMSE 1.24e-2 vs a classical PINN baseline at 2.09e-2). We re-implement the CV ansatz directly in PyTorch (no Strawberry Fields) and reproduce the consistency-loss design on Poisson and heat. **Multi-seed sweep (5 seeds each) on the heat equation refutes the paper's "quantum advantage" claim**: under matched-effort training, the classical PINN reaches 8.74e-3 ± 1.2e-3 RMSE vs the QPINN at 1.23e-2 ± 4.8e-3, i.e. the classical PINN wins by 1.40x on the mean (gap 2.9 PINN-σ) and is ~4x more stable across seeds. We also confirm the paper's central methodological claim (nested autograd costs scale super-linearly with cutoff) via a micro-benchmark — peak RSS delta jumps from 2 MB (cutoff 10) to 30 MB (cutoff 12), validating the consistency-loss trick as a memory optimisation. We ship a MerLin linear-optics adaptation (RMSE 2.4e-4 on Poisson, 600 epochs); a literal MerLin port of the paper's CV architecture is not meaningful because MerLin targets linear optics rather than continuous-variable photonic computing.|
