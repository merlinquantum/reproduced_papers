...
| [Quantum Kitchen Sinks](papers/quantum_kitchen_sinks/). Source: [Wilson_2019](https://arxiv.org/abs/1806.08321) | Reproduces the open-loop QKS recipe — random non-linear quantum features feeding a *linear* classifier. On the synthetic picture-frames dataset we obtain 49.25% (LR baseline) → 100% (2-qubit CNOT QKS, best of σ × E sweep, 3 seeds), matching the paper's >99.9% claim. On the (3,5)-MNIST subset (4 000 train / 1 000 test) the 1-qubit QKS reaches **1.87 ± 0.09%** test error (paper: 3.3%) and the 2-qubit ansatz reaches **1.77 ± 0.24%** (paper: ≈ 1.8%). A new **photonic MerLin adaptation** reaches **99.7%** on picture frames and, in its best enlarged `DUAL_RAIL` setting (`m=6, k=3, E=10000, σ=0.07`), reaches **3.60 ± 0.42%** test error on (3,5)-MNIST, essentially at the LR baseline (**3.80%**) but still behind the best gate-model QKS. The paper's claim that the 2-qubit *CZ* ansatz "has no discrimination" does *not* reproduce in our finite-E experiments — see `INSIGHTS.md`. |
