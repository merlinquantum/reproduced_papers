...
| [QARIMA: A Quantum Approach To Classical Time Series Analysis](papers/QARIMA/). Source: [mohanty_2026](https://arxiv.org/abs/2604.08277) | Quantum-inspired ARIMA (swap-test Q-ACF/PACF + VQC-AR/MA) on five public series (Sunspots, Mauna Loa CO2, Australian beer, woollen yarn, Sydney weather). We show the swap-test cosine reduces to the classical cosine, so QARIMA is a *linear* ARIMA whose coefficients are produced by a VQC. Across all datasets, classical OLS ≈ gate-VQC ≈ **MerLin photonic VQC** to 3–5 significant figures at matched orders. The paper's headline gains (e.g. CO2 MSE 78→10) are a baseline/AR-order artifact: a plain linear AR(14) reproduces the reported "quantum" MSE, and a *fair* seasonal ARIMA reaches MSE 0.40 (25× better than the paper's best quantum model). Verdict: quantum-advantage claim **unsupported under fair baselines**; the photonic translation is feasible but confers no advantage. |
| [Quantum vs. Classical Time-Series Benchmark](papers/variational_qml_ts_benchmark/). Source: [fellner_2026](https://arxiv.org/abs/2504.12416) | Benchmark of 5 variational quantum (d-QNN, ru-QNN, QRNN, QLSTM, le-QLSTM) vs 3 classical (MLP, RNN, LSTM) models on 27 chaotic-time-series forecasting tasks. We reproduce the paper's central ranking **exactly** from the authors' released grid-search CSVs (mean rank: classical 2.90 vs quantum 5.46; ordering LSTM > RNN > le-QLSTM > MLP > d-QNN > ru-QNN > QLSTM > QRNN), and confirm it qualitatively with an independent reduced from-scratch sweep. A code audit found several genuine bugs in the original implementation (ru-QNN exponential-encoding prefactor a factor of 3 too small; QLSTM entangler; Lorenz-QRNN axes; test-set normalization leakage — see `BUGS.md`); fixing them slightly *helps* the quantum side but does not change the conclusion. A MerLin **photonic dressed-QNN** analogue of the best quantum model behaves the same way. Verdict: the paper's negative result (no quantum advantage for time-series forecasting) is **reproduced and robust**. |
