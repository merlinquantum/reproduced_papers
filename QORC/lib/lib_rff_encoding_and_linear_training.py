...
def rff_encoding_and_linear_training(
    ...
    logger.info(
        "Call to rff_encoding_and_linear_training: n_rff_features={}, sigma={}, reg_C={}, seed={}, b_optim_via_sgd={}".format(
    ...
    # Transformation RFF
    rff = RBFSampler(gamma = 1.0 / (2 * sigma**2), n_components = n_rff_features, random_state = run_seed)
    ...
    all_train_data = np.hstack([scaled_train_data, train_data_rff])  # Shape: [n_samples, 28x28 + n_rff_features]
    ...
    if (b_optim_via_sgd):
        ...
        clf = SGDClassifier(
            ...
            alpha=1.0 / (1.0 * n_samples * regularization_c),          # 1/(n_samples * C), avec C=1.0
        ...
        clf = LinearSVC(multi_class='ovr', random_state = run_seed, C = regularization_c)    # One-Versus-Rest: Entrainer 1 classifier binaire par classe puis choisir la classe de marge max
    ...
    train_acc = int(10000. * accuracy_score(train_label, train_model_pred)) / 10000.
    test_acc  = int(10000. * accuracy_score(test_label,  test_model_pred)) / 10000.


    duration_calcul_rff_features = int(100.0 * (time_t3 - time_t2)) / 100.0
    logger.info(
        "Duration - RFF features encoding: {}s".format(
