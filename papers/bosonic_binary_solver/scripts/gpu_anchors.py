...
def check_boson_matches_perceval(device, m=12, delays=(1, 3, 9), shots=40000):
    ...
    if ClickSource is None:
        print(
            "  boson vs Perceval: SKIPPED (perceval-quandela not installed on this host)"
        )
        return
    theta = (
    ...
    reference = ClickSource(m, delays, input_state, source="boson").draw(
        theta, shots, np.random.default_rng(1)
    )
    unitaries = unitary(torch.tensor(theta, dtype=torch.float64), m, delays)[None].to(
        device
    )
    ...
    print(
        f"  boson vs Perceval: max per-mode rate gap {gap:.4f} (tolerance {tolerance:.4f}) "
...
def check_batched_matches_per_circuit(device, m=10, delays=(1, 3), shots=20000):
    ...
    together = (
        draw_clicks(
            ...
            modes,
            shots,
            torch.Generator(device=device).manual_seed(2),
            "boson",
            m,
        )
        .to(torch.float64)
    ...
    separate = np.stack(
        [
            draw_clicks(
                ...
                modes,
                shots,
                torch.Generator(device=device).manual_seed(3 + i),
                "boson",
                m,
            )
            .to(torch.float64)
    ...
    print(
        f"  batched vs per-circuit: max per-mode rate gap {gap:.4f} (tolerance {tolerance:.4f})"
    )
...
def check_accounting(device):
    ...
    result = solve_batch(
        cost_batch, 8, seeds, updates=4, samples=4, source="bernoulli", device=device
    )
    print(
        f"  accounting: {result['evaluations']} evaluations vs bound {result['budget']}"
    )
...
def check_end_to_end(device):
    ...
    result = solve_batch(
        cost_batch, 10, seeds, updates=200, samples=50, source="boson", device=device
    )
    found = sum(abs(float(b) - o) < 1e-9 for b, o in zip(result["best_cost"], optima))
    print(
        f"  end to end m=10: {found}/4 optimal, E[C] {np.mean(result['history'][:10]):.1f} "
...
def main():
    ...
    parser.add_argument(
        "--skip-sampler",
        action="store_true",
        help="skip the checks needing cliffordgpu",
    )
