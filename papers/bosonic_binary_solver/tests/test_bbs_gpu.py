...
def test_batched_costs_match_the_scalar_definition():
    ...
    for family, m in (("knapsack", 10), ("tsp", 10)):
        ...
        expected = np.stack([build_problem(family, m, seed)[1](bits[slot]) for slot, seed in enumerate(seeds)])
...
def test_shifted_angles_layout():
    ...
    for k in range(3):
        assert stacked[0, 1 + 2 * k, k] == pytest.approx(float(theta[0, k]) + math.pi / 2)
        assert stacked[0, 2 + 2 * k, k] == pytest.approx(float(theta[0, k]) - math.pi / 2)
...
def test_batched_and_scalar_sources_agree_in_distribution():
    ...
    for source in ("distinguishable", "bernoulli"):
        scalar = ClickSource(m, delays, input_state, source=source).draw(theta, shots, np.random.default_rng(1)).mean(0)
        batched = draw_clicks(unitaries, modes, shots, generator, source, m).to(torch.float64).mean(dim=(0, 1)).numpy()
...
def test_freeze_theta_holds_the_interferometer_still():
    ...
    seeds = [0, 1]
    cost_batch, _ = build_batch("knapsack", 8, seeds, torch.device("cpu"))
    frozen = solve_batch(cost_batch, 8, seeds, updates=6, samples=8, source="bernoulli", freeze_theta=True)
    trained = solve_batch(cost_batch, 8, seeds, updates=6, samples=8, source="bernoulli", freeze_theta=False)
...
def test_bernoulli_rate_scale_moves_click_density_only():
    ...
    for scale in (0.9, 1.0, 1.2):
        result = solve_batch(cost_batch, 10, seeds, updates=4, samples=32, source="bernoulli", rate_scale=scale)
