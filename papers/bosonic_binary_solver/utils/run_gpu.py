...
def run_key(cfg, seed):
    ...
    return "|".join(
        [
            ...
            method["name"],
            method.get("source", "-"),
            f"rate{method.get('rate_scale', 1.0)}",
            method.get("topology", "chain"),
            f"N{training['updates']}",
            f"S{training['samples']}",
...
def main():
    ...
    parser.add_argument(
        "--chunk", type=int, default=20, help="instances solved in lockstep"
    )
    ...
    if device.type == "cuda":
        ...
        print(
            f"device: {torch.cuda.get_device_name(0)}, {free / 2**30:.1f} of {total / 2**30:.1f} GiB free",
            flush=True,
        )
    ...
    for config_path in args.configs:
        ...
        todo = [
            problem.get("instance_seed_base", 0) + i
        ...
        print(
            ...
            f"{budget / 2**m:.3g} of the space; {len(todo)} instances to run",
            flush=True,
        )

        for start in range(0, len(todo), args.chunk):
            seeds = todo[start : start + args.chunk]
            cost_batch, optima = build_batch(
                problem["family"], m, seeds, device, **problem.get("kwargs", {})
            )
            began = time.time()
            result = solve_batch(
                cost_batch,
            ...
            print(
                f"  instances {seeds[0]}-{seeds[-1]}: {optimal}/{len(seeds)} optimal, "
                f"{elapsed:.1f}s ({elapsed / len(seeds):.1f}s per instance), "
                f"mean clicks {np.mean(result['mean_clicks']):.2f}/{m}",
                flush=True,
            )
    handle.close()
