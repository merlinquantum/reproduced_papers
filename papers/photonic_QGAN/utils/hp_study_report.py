...
class HPStudyReporter:
    ...
    def _generate_markdown_report(self) -> str:
        ...
        final_stage_results = self.cv_results[self.cv_results['param_opt_iter_num'] == 600].nlargest(3, 'mean_test_score')
...
"""
        ...
        for i, (_, row) in enumerate(final_stage_results.iterrows(), 1):
            report += f"| {i} | {row['mean_test_score']:.6f} | {row['std_test_score']:.6f} | "
...
"""
        ...
        if 'top_unique_any_resource' in self.analysis:
            ...
            for i, cand in enumerate(self.analysis['top_unique_any_resource'][:10], 1):
                ...
                report += f"{params.get('d_steps', '-')} | {params.get('g_steps', '-')} |\n"
...
"""
    ...
    def create_summary_statistics_plot(self, save_path=None):
        ...
        ax1.hist(self.cv_results['mean_test_score'], bins=30, alpha=0.7, color='steelblue', edgecolor='black')
        ax1.axvline(self.best_score, color='red', linestyle='--', linewidth=2, label=f'Best: {self.best_score:.4f}')
        ...
        for resource in self.resource_levels:
            data = self.cv_results[self.cv_results['param_opt_iter_num'] == resource]['mean_test_score']
            ax2.scatter([resource] * len(data), data, alpha=0.5, s=30, label=f'{resource} iters')
        ...
        ax3.hist(self.cv_results['mean_fit_time'], bins=20, alpha=0.7, color='coral', edgecolor='black')
        ...
        params_to_plot = ['param_lrG', 'param_lrD', 'param_adam_beta1', 'param_adam_beta2', 
                         'param_d_steps', 'param_g_steps']
        titles = ['Gen. Learning Rate', 'Disc. Learning Rate', 'Adam β₁', 'Adam β₂', 
                 'Disc. Steps', 'Gen. Steps']
        
        for idx, (param, title) in enumerate(zip(params_to_plot, titles), 4):
            ax = plt.subplot(3, 3, idx)
            data_grouped = self.cv_results.groupby(param)['mean_test_score'].agg(['mean', 'std'])
            x_pos = range(len(data_grouped))
            ax.bar(x_pos, data_grouped['mean'], yerr=data_grouped['std'], 
    ...
    def print_executive_summary(self):
        ...
        print(f"  Average Training Time:    {self.cv_results['mean_fit_time'].mean():.1f}s")
        print(f"  Median Training Time:     {self.cv_results['mean_fit_time'].median():.1f}s")
        ...
        for idx, (_, row) in enumerate(top_3.iterrows(), 1):
            ...
            print(f"      lrG={row['param_lrG']}, lrD={row['param_lrD']}, " +
...
def main():
    ...
    hp_study_dir = project_root / "papers" / "photonic_QGAN" / "results" / "run_20260213-104332" / "hp_study"
