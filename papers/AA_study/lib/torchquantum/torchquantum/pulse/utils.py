...
def H_2q_example(pulse, dt):
    def H(t):
        t_ind = (t / dt).long()
        interaction = (
            -1.548 * torch.kron(sigmai(), sigmax())
            - 0.004 * torch.kron(sigmai(), sigmay())
            - 0.006 * torch.kron(sigmai(), sigmaz())
            + 5.316 * torch.kron(sigmaz(), sigmax())
            - 0.225 * torch.kron(sigmaz(), sigmay())
            - 0.340 * torch.kron(sigmaz(), sigmaz())
        )
        if t_ind >= len(pulse):
