...
- **Main claims:** QR1/QR2 outperform nine classical benchmarks on MSE and QLIKE
  at one-step-ahead horizon (Table II); QR2 is the single best model; the paper also
  reports strong results at `S = 5`; `n1 = 7` input features is optimal.
  <!-- CHANGED: removed the unqualified description of the paper's S=5 result as a closed-loop result. -->
- **Bottom line:** `partially reproduced` — `S = 1` reproduces closely; the
  `S = 5` result depends materially on the forecasting convention and our corrected
  V2 implementation gives stronger performance than the paper.
  <!-- CHANGED: updated to reflect the resolved V2 S=5 implementation difference. -->
- **Main takeaways:**
...
- **Targeted:** QR1 and QR2 at paper settings (`S = 1`); the HAR, HARX, AR1, AR3,
  ...
  multi-step horizon; the full 100-instance reservoir distribution; corrected and
  selection-matched fair baselines; wrapper forward feature selection (QR1); a
  MerLin photonic counterpart.
  <!-- CHANGED: 'closed-loop horizon' softened to 'multi-step horizon' because the
  paper does not specify the exact S=5 forecasting convention well enough to label
  the authors' protocol definitively. -->
...
| Missing details / assumptions | Feature scaling stated as `[-pi, pi]` but the released code uses `pi * x` on min-max normalised columns; `J` stated as `U[0,1]` but the code rescales to unit spectral radius; QLIKE formula in the text differs from the code; MCS bootstrap seed unset; the exact `S = 5` forecasting convention is not sufficiently specified; the selection split for "best-performing reservoir" is unstated | Reference-code behaviour adopted wherever it disagrees with the text. For V2, `S = 5` is implemented as an actual five-step forecast: for target `t`, forecasting starts at `t-4`, with the forecast advanced through the five-step horizon according to the model's closed-loop logic | The earlier V1 treatment instead learned a direct mapping from the state at an earlier origin to the target five months later. This distinction explains why the two implementations can produce materially different `S = 5` numbers. Exact author indexing remains undocumented. <!-- CHANGED: explicit V1 vs V2 S=5 treatment added. --> |
...
- **Result status:** metric agreement `reproduced` at `S = 1`; trend agreement
  `not reproduced`; claim support `unsupported`; `S = 5` `not numerically matched
  under the V2 multi-step implementation`; feature-selection claims `reproduced`.
  <!-- CHANGED: S=5 is no longer described as simply unresolved: the implementation
  difference is now identified at the level of forecasting convention, although the
  authors' exact intended indexing is still not fully documented. -->
...
- **Figures reproduced:** Table II (`S = 1` fully; `S = 5` evaluated but not
  numerically matched to the paper under the V2 multi-step convention), Table III
  (Diebold-Mariano matrix), Fig. 6(a) (feature-count curve and optimal set, exactly).
  Fig. 8 (Shapley) and Fig. 9 (concentration) not attempted.
  <!-- CHANGED: updated S=5 status to distinguish a coherent V2 result from an exact
...
- **Explanation of differences.** The quantum and autoregressive rows match to
  ...
  verdict. For `S = 5`, the implementations are conceptually different: the earlier
  V1 approach effectively learns a direct mapping from the state available at the
  forecast origin to the value five months later, whereas V2 starts five months
  before the target and actually advances the forecast through the five-step
  horizon. The corrected V2 diagnostic run gives QR1 `0.118771` and QR2 `0.133029`,
  compared with paper values `0.1556` and `0.1663`. The stronger V2 result is
  therefore not evidence of a simple bug fix that reproduces the paper; it is the
  result of using a more literal multi-step forecasting treatment under an
  incompletely specified `S = 5` protocol. Exact author indexing remains unknown.
  <!-- CHANGED: replaced the previous generic 'unresolved closed-loop protocol'
  explanation with the identified V1/V2 conceptual distinction and updated QR
  diagnostic values. -->
...
- **What we conclude.** The paper is *reproducible* but its central comparative
  ...
  usefully to photonics. For `S = 5`, V2 should be treated as a reasonable,
  more literal multi-step implementation rather than a numerical reproduction of
  the paper's result: it performs better, but the source does not specify enough
  detail to prove that this is the authors' intended convention.
  <!-- CHANGED: added the minimal conclusion needed to explain the improved V2 S=5
  result without claiming an exact reproduction. -->
...
- **What we could not do next.** A definitive numerical reproduction of the
  paper's `S = 5` comparison without the authors' exact forecasting/indexing
  convention; and exact MCS p-value reproduction, because the bootstrap seed is
  unset upstream.
  <!-- CHANGED: clarified that the remaining S=5 limitation is exact matching to
  the authors, not the ability to implement and evaluate a coherent five-step forecast. -->
