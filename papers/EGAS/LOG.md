...
## Reproduced Figures (all paper figures plotted → results/)
Generated by `utils/plot_paper_figures.py` from saved metrics.json + accuracies.npz:
Table I → `table1_wasserstein.png`; Fig 1 → `fig1_tracedist_vs_w1.png`; Fig 2 (schematic) →
`fig2_egas_schematic.png`; Fig 3 → `fig3_deltaE_per_candidate.png` (faithful 10G+10B with error
bars via `utils/run_fig3_repeats.py`); Fig 4 → `fig4_deltaE_groups.png`; Fig 5 →
`fig5_win_tie_loss.png`; Fig 6 → `fig6_iqr.png`; Fig 7 → `fig7_accuracy_heatmap.png`.
Figs 3–7 use the 3 EGAS datasets actually run (PW, WQ, MGT) — reduced vs paper's 8 datasets.

## Reproduced Figures and Tables (detail)
