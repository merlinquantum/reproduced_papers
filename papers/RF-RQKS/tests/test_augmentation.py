...
def _write_source_dataset(root: Path) -> None:
    for split, pair_count in (("train", 2), ("test", 1)):
        ...
        with (split_root / "metadata.csv").open("w", newline="", encoding="utf-8") as file:
            writer = csv.DictWriter(file, fieldnames=["sample_index", "pair_index", "label"])
            writer.writeheader()
...
def test_augmentation_reaches_target_counts_and_preserves_pairs(tmp_path: Path) -> None:
    ...
    augment_processed_dataset(source_root, output_root, train_pair_count=5, test_pair_count=3)

    for split, pair_count in (("train", 5), ("test", 3)):
        split_root = output_root / split
        assert np.load(split_root / "spectrograms.npy").shape == (2 * pair_count, 6, 8)
        assert np.array_equal(np.load(split_root / "labels.npy"), np.tile([0, 1], pair_count))
        ...
        for pair_index in range(pair_count):
            ...
            assert pair_rows[0]["pair_index"] == pair_rows[1]["pair_index"] == str(pair_index)
