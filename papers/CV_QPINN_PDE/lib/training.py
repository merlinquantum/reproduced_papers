...
def cosine_annealing_with_warm_restarts(
    initial_lr: float, t: int, period: int, min_lr: float = 1e-5
) -> float:
    ...
    return min_lr + 0.5 * (initial_lr - min_lr) * (
        1 + math.cos(math.pi * t_in_cycle / period)
    )
...
def train_poisson(
    ...
    x_bc: tuple[torch.Tensor, torch.Tensor],
    *,
    loss_fn: Callable,
    lr: float,
    epochs: int,
    ...
    for epoch in range(epochs):
        ...
        if epoch % log_every == 0 or epoch == epochs - 1:
            print(
                f"  [epoch {epoch:5d}] total={parts['total'].item():.4e} "
    ...
    return {
        "history": history,
        "best": {"epoch": best["epoch"], "loss": best["loss"]},
...
def train_heat(
    ...
    xt_ic: torch.Tensor,
    T_ic: torch.Tensor,
    xt_bc: tuple[torch.Tensor, torch.Tensor],
    *,
    loss_fn: Callable,
    lr: float,
    pretrain_epochs: int,
    ...
    for epoch in range(epochs):
        ...
        if epoch % log_every == 0 or epoch == epochs - 1:
            print(
                f"  [full {epoch:5d}] total={parts['total'].item():.4e} "
