...
def hidden_layers_for_param_count(target_params: int, in_features: int,
                                  out_features: int = 2,
                                  hidden_layers: int = 1) -> list[int]:
    ...
    """
    if hidden_layers == 1:
        h = max(1, round((target_params - out_features) / (in_features + 1 + out_features)))
        return [h]
