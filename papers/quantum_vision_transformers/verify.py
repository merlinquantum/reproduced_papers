...
from lib.data import HierarchicalPatchEmbed  # noqa: E402
...
from lib.photonic_primitives import (  # noqa: E402
    TrainableInterferometer,
    normalize_for_encoding,
)
from lib.structured_circuits import (  # noqa: E402
...
check(
    "Output sums to ≈ 1 (probability conservation)",
    torch.allclose(y.sum(dim=-1), ones(y, 3), atol=1e-3),
)
y.sum().backward()
check(
    "Gradients flow",
    any(p.grad is not None and p.grad.abs().sum() > 0 for p in V.parameters()),
)
...
check(
    "Softmax sums to 1", torch.allclose(A_soft.sum(-1), ones(A_soft, 2, 5), atol=1e-5)
)
...
check(
    "Has sector_masses dict", isinstance(info_df, dict) and "sector_masses" in info_df
)
...
check(
    f"Params tied ({len(independent_2ph)} independent in 2ph)",
    len(independent_2ph) == 0,
)
...
print("\n— HierarchicalPatchEmbed —")
hpe = HierarchicalPatchEmbed(
    img_size=28, in_channels=3, n_regions_per_side=2, n_patches_per_side=2, embed_dim=8
)
...
check(
    "Butterfly pairing schedule matches radix-2 layout",
    spec.pairings
    == [
        [(0, 1), (2, 3), (4, 5), (6, 7)],
        [(0, 2), (1, 3), (4, 6), (5, 7)],
...
check(
    "Butterfly parameter count matches schedule",
