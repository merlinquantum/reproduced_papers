...
class TrainableInterferometer(nn.Module):
    ...
    def __init__(self, m: int, n_photons: int = 1, name: str = "U",
        ...
        self.layer = ML.QuantumLayer(
            builder=builder, n_photons=n_photons,
...
class CompoundSectorReadout(nn.Module):
    ...
    def __init__(self, n_patches: int, d_feat: int,
        ...
        for idx, key in enumerate(output_keys):
            ...
            if sum(occ[:n_patches]) == 1 and sum(occ[n_patches:]) == 1:
                ...
                fi.append(next(i for i in range(n_patches, m) if occ[i] == 1) - n_patches)
...
class FullSectorReadout(nn.Module):
    ...
    def __init__(self, n_patches: int, d_feat: int,
        ...
        for idx, key in enumerate(output_keys):
            ...
            if pc == 1 and fc == 1:
                ...
                cross_fi.append(next(i for i in range(n_patches, m) if occ[i] == 1) - n_patches)
            elif pc == 2 and fc == 0:
                ...
                if len(modes) == 2:
                    ff_idx.append(idx); ff_i.append(modes[0]); ff_j.append(modes[1])
                elif len(modes) == 1:
                    ff_idx.append(idx); ff_i.append(modes[0]); ff_j.append(modes[0])

        for name, t in [("cross_idx", cross_idx), ("cross_pi", cross_pi),
                         ("cross_fi", cross_fi), ("pp_idx", pp_idx),
                         ("pp_i", pp_i), ("pp_j", pp_j), ("ff_idx", ff_idx),
                         ("ff_i", ff_i), ("ff_j", ff_j)]:
    ...
    def forward(self, probs):
        ...
        A[..., self.pp_j, self.pp_i] = pv  # symmetrise (diagonal self-assigns harmlessly)
...
class TripleSectorReadout(nn.Module):
    ...
    def __init__(self, n_regions: int, n_patches: int, d_feat: int,
                 output_keys: list[tuple[int, ...]],
        ...
        for idx, key in enumerate(output_keys):
            ...
            if rc == 1 and pc == 1 and fc == 1:
                ...
                tc_pi.append(next(i for i in range(r_end, p_end) if occ[i] == 1) - r_end)
                ...
                if len(pmodes) == 2:
                    rpp_idx.append(idx); rpp_ri.append(ri)
                    rpp_pi.append(pmodes[0]); rpp_pj.append(pmodes[1])
                elif len(pmodes) == 1 and occ[r_end + pmodes[0]] == 2:
                    rpp_idx.append(idx); rpp_ri.append(ri)
                    rpp_pi.append(pmodes[0]); rpp_pj.append(pmodes[0])

        for name, t in [("tc_idx", tc_idx), ("tc_ri", tc_ri),
                         ("tc_pi", tc_pi), ("tc_fi", tc_fi)]:
            self.register_buffer(name, torch.tensor(t, dtype=torch.long))
        if extract_rpp:
            for name, t in [("rpp_idx", rpp_idx), ("rpp_ri", rpp_ri),
                             ("rpp_pi", rpp_pi), ("rpp_pj", rpp_pj)]:
                self.register_buffer(name, torch.tensor(t, dtype=torch.long))
    ...
    def forward(self, probs):
        ...
        T = torch.zeros(*bs, self.r, self.p, self.d, dtype=probs.dtype, device=probs.device)
        ...
        if self.extract_rpp and hasattr(self, "rpp_idx") and self.rpp_idx.numel() > 0:
            ...
            A_rpp = torch.zeros(*bs, self.r, self.p, self.p, dtype=probs.dtype, device=probs.device)
