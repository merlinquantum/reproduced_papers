...
def evaluate(model, loader, device, n_classes):
    ...
    return {"acc": acc, "auc": _auc(probs, labels, n_classes),
...
def train(model, train_loader, val_loader, test_loader,
          n_classes, config, outdir, device, resume_checkpoint: str | None = None,
    ...
    if lr_quantum is not None:
        ...
        classical_params = [p for n, p in model.named_parameters()
    ...
    if resume_checkpoint is not None:
        start_epoch, best_auc, best_epoch, history, elapsed_before_resume, resumed_from_checkpoint = _load_resume_checkpoint(
            model=model,
            opt=opt,
            sched=sched,
    ...
    # ── header ──
    param_counts = model.count_trainable_params() if hasattr(model, "count_trainable_params") else {}
    lr_str = f"lr={lr}"
    if lr_quantum is not None:
        lr_str += f"  lr_quantum={lr_quantum}"
    logger.info("=" * 65)
    logger.info(f"Model {model_type}  |  dataset={config.get('dataset','?')}  "
    ...
    if resumed_from_checkpoint:
        logger.info(f"Resuming from {resume_checkpoint}  |  next epoch={start_epoch}  |  best val AUC {best_auc:.4f} @ ep {best_epoch}")
    logger.info("=" * 65)
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for bi, batch in enumerate(train_loader):
            ...
            if n_batches > 4 and (bi + 1) % max(1, n_batches // 4) == 0:
                logger.debug(f"  batch {bi+1}/{n_batches}  "
                             f"loss={loss.item():.4f}")
        ...
        rec = {
            ...
            "lr_classical": opt.param_groups[1]["lr"] if lr_quantum is not None else None,
        ...
        parts = [f"Ep {epoch:3d}/{epochs}",
                 ...
                 f"| {t_epoch:.1f}s"]
        _sector_abbrev = {
            "sector_mass_cross": "C", "sector_mass_pp": "PP", "sector_mass_ff": "FF",
            "sector_mass_triple_cross": "TC", "sector_mass_rpp": "RPP",
        }
        sector_keys = [k for k in _sector_abbrev if k in rec]
        if sector_keys:
            sector_str = " ".join(f"{_sector_abbrev[k]}={rec[k]:.2f}" for k in sector_keys)
            parts.append(f"| sectors {sector_str}")
        logger.info("  ".join(parts))

        if epoch % 10 == 0:
            elapsed = timedelta(seconds=int(time.time() - t_start))
            logger.info(f"  ── {elapsed} elapsed  |  best val AUC {best_auc:.4f} @ ep {best_epoch}")
    ...
    logger.info(f"Test: acc={test['acc']:.4f}  auc={test['auc']:.4f}  "
                f"(best val AUC {results['best_val_auc']:.4f} @ ep {best_epoch})")
    logger.info(f"Total time: {timedelta(seconds=int(total_time))}  |  saved → {results_path}")
    logger.info("=" * 65)
