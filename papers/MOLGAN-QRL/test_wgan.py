...
def models(gan_config):
    ...
    G = Generator(
        gan_config["g_conv_dims"], gan_config["z_dim"], 
        gan_config["N_nodes"], gan_config["N_bonds"], gan_config["N_atoms"], 0.0
    )
    D = Discriminator(
...
def test_forward_shapes(models, gan_config):
    ...
    assert edges_logits.shape == (gan_config["batch_size"], gan_config["N_nodes"], gan_config["N_nodes"], gan_config["N_bonds"])
    assert nodes_logits.shape == (gan_config["batch_size"], gan_config["N_nodes"], gan_config["N_atoms"])

    # 2. Gumbel-Softmax
    fake_adj = F.gumbel_softmax(edges_logits, tau=gan_config["tau"], hard=False, dim=-1)
    fake_nodes = F.gumbel_softmax(nodes_logits, tau=gan_config["tau"], hard=False, dim=-1)

    # 3. Forward Discriminateur
    d_out = D(fake_adj, None, fake_nodes)
    assert d_out.shape == (gan_config["batch_size"], 1), f"Sortie D inattendue : {d_out.shape}"
...
def test_discriminator_gradients(models, gan_config):
    ...
    real_adj = torch.randn(batch_size, gan_config["N_nodes"], gan_config["N_nodes"], gan_config["N_bonds"])
    ...
    fake_nodes = F.gumbel_softmax(nodes_logits, tau=gan_config["tau"], hard=True, dim=-1)
    ...
    for name, param in G.named_parameters():
        assert param.grad is None, f"Erreur fatale : G a reçu un gradient pendant l'entraînement de D ({name})"
...
def test_micro_convergence(models, gan_config):
    ...
    for _ in range(3):
        ...
        d_loss = -torch.mean(D(real_adj, None, real_nodes)) + torch.mean(D(fake_adj.detach(), None, fake_nodes.detach()))
        d_loss.backward()
        opt_D.step()
        ...
        fake_nodes_g = F.gumbel_softmax(nodes, tau=gan_config["tau"], hard=False, dim=-1)
    ...
    assert not torch.isnan(d_loss), "Explosion numérique (NaN) dans la loss du Discriminateur."
    assert not torch.isnan(g_loss), "Explosion numérique (NaN) dans la loss du Générateur."