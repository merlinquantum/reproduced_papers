...
class HybridMLPModel(nn.Module):
    ...
    """
    def __init__(self, q_output_size: int, nb_photons: int, nb_modes: int, 
                 hidden_sizes: list[int], final_output_size: int):
        super().__init__()
        self.quantum_layer = QLayer(q_output_size, nb_photons, nb_modes)
        self.mapping = MappingModel(q_output_size, hidden_sizes, final_output_size)
...
class HybridMPSModel(nn.Module):
    ...
    """
    def __init__(self, q_output_size: int, nb_photons: int, nb_modes: int, 
                 bond_dim: int, final_output_size: int):
        super().__init__()
        self.quantum_layer = QLayer(q_output_size, nb_photons, nb_modes)
        self.mapping = MPS(input_dim=q_output_size, output_dim=final_output_size, bond_dim=bond_dim)
...
class TorchQuantumModel(nn.Module):
    ...
    Args:
        ...
        final_output_size (int): Final output dimension.
    """
    def __init__(self, q_output_size: int, n_qubit: int, q_depth: int, 
                 hidden_sizes: list[int], final_output_size: int):
        super().__init__()
        self.n_qubit = n_qubit
        self.q_depth = q_depth
...
class classic_model(nn.Module):
    ...
    Args:
        ...
        final_output_size (int): Final output dimension.
    """
    def __init__(self, layer_dim: list[int], hidden_sizes: list[int], final_output_size: int):
        super().__init__() 
...
def rl_agent_forward(state_tensor: torch.Tensor, generated_weights: torch.Tensor, 
                    input_dim: int = 4, output_dim: int = 2) -> torch.Tensor:
    """
...
def train_cartpole_qtrl(hybrid_model: nn.Module, num_episodes: int = 1000, 
...
def train_minigrid_qtrl(hybrid_model: nn.Module, num_episodes: int = 1000, 
                       learning_rate: float = 0.005, seed: int = 42) -> float:
    """
    ...
    print(f"MiniGrid training begins...")
    print(f"Required parameters : {total_weights_needed} ({state_dim} inputs x {action_dim} actions)")
...
def train_environment(hybrid_model: nn.Module, num_episode: int = 1000, 
                     learning_rate: float = 0.005, env_name: str = "CartPole-v1", 
                     seed: int = 42) -> float:
    """
    ...
    print(f"Starting training on {env_name} | Required weights : {total_weights_needed}")
