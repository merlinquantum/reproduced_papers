...
from src.gates import VectorLoader
from src.noise import GateNoise, MeasurementNoise
from src.utils import create_circuit, get_angles
...
def quantum_inner_merlin(x, y, layer=None):
    x_forward = torch.cat((2*torch.tensor(get_angles(x)[0]) / torch.pi,
               (-2*torch.flip(torch.tensor(get_angles(y)[0]) / torch.pi, dims=[0]))))
    result = layer(x_forward).sqrt()
    if len(result.shape) == 1:
        return result[0]
    return result[0, 0]


def quantum_inner_pcvl(x, y, repetitions=1000, error_rate=0, error_mitigation=True, layer=None):
    if error_rate == 0.:
        error_mitigation = False
    n = len(x)
...
def quantum_distance(x,
    ...
    inner = quantum_inner(x, y, repetitions=repetitions, error_rate=error_rate, error_mitigation=error_mitigation)
    dist = np.sqrt(norm_x**2 + norm_y**2 - 2 * norm_x * norm_y * inner)
    return dist


def quantum_distance_ml(x,
                     y,
                     layer=None):
    norm_x = np.linalg.norm(x)
    norm_y = np.linalg.norm(y)
    c_dist = np.linalg.norm(x - y)
    inner = quantum_inner_merlin(x, y, layer=layer)
